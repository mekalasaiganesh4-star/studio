...
export default function FoodDetailPage() {
  ...
  useEffect(() => {
    if (item) {
      setPreparedDate(new Date(item.preparedTime).toLocaleString());
      setExpiryDate(new Date(item.expiryTime).toLocaleString());
    }
  }, [item]);
          ...
          <div className="grid md:grid-cols-2 gap-8 lg:gap-16">
            <div className="aspect-w-16 aspect-h-9 relative rounded-lg overflow-hidden shadow-lg">
              ...
              {item.verified && (
                <Badge
                  variant="default"
                  className="absolute top-4 right-4 bg-primary/80 backdrop-blur-sm text-primary-foreground"
                >
            ...
            <div>
              ...
              <Card className="mt-6 bg-secondary/30">
                <CardContent className="p-6">
                  <div className="flex items-baseline gap-2">
                    ...
                    <span className="text-xl line-through text-muted-foreground flex items-center">
                  ...
                  <p className="text-green-600 font-semibold mt-1">
                    You save{' '}
                    {Math.round(
                      100 - (item.discountedPrice / item.originalPrice) * 100
                    )}
                    %
                  </p>
              ...
              <div className="mt-8 space-y-4 text-sm text-foreground">
                <h3 className="text-lg font-semibold font-headline border-b pb-2">
                  Item Details
                </h3>
